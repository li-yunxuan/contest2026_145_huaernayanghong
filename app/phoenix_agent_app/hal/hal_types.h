...
/** 环境温湿度传感器数据 (板载 SHTC3 等器件) */
typedef struct {
    float temperature_c;           /**< 环境温度摄氏度 (如 26.5) */
    float humidity_pct;            /**< 相对湿度百分比 0~100 (如 60.0) */
    bool is_valid;                 /**< 传感器读数是否有效就绪 */
    uint64_t timestamp_us;         /**< 采样微秒级时间戳 */
} hal_env_data_t;

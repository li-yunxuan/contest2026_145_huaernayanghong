...
def is_spam_log_line(line: str) -> bool:
    """过滤高频 Web 轮询日志（如 /api/status, /api/audio/status 等及 Sent 响应）"""
    if "PhoenixWeb" in line:
        if "GET /api/" in line or "Sent " in line:
            return True
    return False

def fetch_app_log(verbose=False):
    ...
    if not content:
        # Fallback to cat
        content = run_adb_cmd("cat /data/phoenix/logs/phoenix.log")
    
    if not verbose:
        lines = [l for l in content.splitlines() if not is_spam_log_line(l)]
        return "\n".join(lines)
    return content
...
def follow_logs(interval=0.5, verbose=False):
    print("📡 [实时日志流已启动] 正在流式监听 Phoenix 应用层运行日志... (按 Ctrl+C 退出)")
    if not verbose:
        print("💡 [提示] 已默认过滤 Web 高频轮询 (如 /api/status 等)，使用 -v/--verbose 可查看完整日志")
    print("=" * 70)
    last_size = 0
...
def main():
    ...
    parser.add_argument("-v", "--verbose", action="store_true", help="显示全部日志（包含 Web 高频心跳轮询）")
    ...
    if args.follow:
        follow_logs(verbose=args.verbose)
        return
    ...
    if args.app:
        ...
        print("=" * 70)
        log = fetch_app_log(verbose=args.verbose)
    ...
    print("-----------------------------------------------------------------------")
    app_log = fetch_app_log(verbose=args.verbose)
